...
The converter:
1. Reads level list from dynamic_item_spawn_locations.ltx
2. Creates ONE space_restrictor per level (name: sr_item_spawner_<level>)
...
class ItemRestrictorConverter:
    ...
    def _load_level_list(self, path: Path) -> None:
        """Load level list from dynamic_item_spawn_locations.ltx."""
        if not path.exists():
            log(f"Warning: {path} not found, no item restrictors will be created")
            return
        ...
        log(f"  Loaded {len(self.levels)} levels for item restrictors")

    def get_levels(self) -> List[str]:
        """Get list of all levels that need item restrictors."""
        return self.levels
    ...
    def _create_spawn_packet(self, level_name: str, location: LevelRestrictorInfo) -> bytes:
        ...
        # Entity name (stringZ) - one per level: sr_item_spawner_<level>
        entity_name = f"sr_item_spawner_{level_name}"
...
def create_item_restrictors_for_level(
    converter: ItemRestrictorConverter,
    level_name: str
) -> List[Tuple[bytes, Optional[bytes]]]:
    """
    Create item restrictor packet for a given level.

    Args:
        converter: ItemRestrictorConverter instance
        level_name: Name of the level
