...
> **⚠️ Save Game Compatibility Warning**
>
> Rebuilding all.spawn **will break existing save games**. This is unavoidable - save files contain references to GVIDs (Game Vertex IDs) that change when the game graph is rebuilt. Players must start a new game after installing your mod.
>
> See [New Game vs Loading a Save](ALL_SPAWN_FORMAT.md#new-game-vs-loading-a-save) for technical details on why this happens.
...
> **Understanding spawn data:** The all.spawn file contains entity definitions with position-based vertex IDs (GVIDs) that must match the game graph. See [all.spawn Format](ALL_SPAWN_FORMAT.md) for details on why merging with original data preserves mod compatibility.
...
> **Inform Your Users:** Make sure your mod description clearly states that **a new game is required**. Existing saves are not compatible with rebuilt all.spawn files because save files contain GVID references that become invalid when the game graph changes. See [New Game vs Loading a Save](ALL_SPAWN_FORMAT.md#new-game-vs-loading-a-save) for why this is unavoidable.

## Troubleshooting

**Game crashes or behaves strangely when loading a save after installing the mod**
- This is expected. Rebuilding all.spawn changes all GVIDs in the game graph, which breaks save file references.
- Save files store entity positions and AI state using GVID values from the old game graph. When you rebuild all.spawn, these GVIDs point to different (or non-existent) locations.
- **Solution:** Players must start a new game. There is no way to make rebuilt all.spawn files compatible with existing saves.
- See [New Game vs Loading a Save](ALL_SPAWN_FORMAT.md#new-game-vs-loading-a-save) for technical details.
